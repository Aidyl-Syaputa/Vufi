...
api_url = os.getenv("HERCAI_API_URL", "https://hercai.onrender.com/v3-beta/hercai?question=")
...
def generate_response(user_input: str) -> Response:
    ...
    response = requests.get(f"{api_url}?question={lc_input}")
...
def api():
    try:
        ...
        if not user_input:
            return jsonify({'error': 'Parameter "input" tidak valid'}), 400
